...
(function () {
  ...
  var MAP_PIN_MOVE = {
    x: {
      min: window.map.MAP_SIZES.width - window.map.MAIN_PIN.sizes.width / 2 - 1,
      max: 0 - window.map.MAIN_PIN.sizes.width / 2 - 1
    },
    y: {
      min: window.map.MAP_SIZES.coordStart - window.map.MAIN_PIN.sizes.height,
      max: window.map.MAP_SIZES.coordEnd - window.map.MAIN_PIN.sizes.height
    }
  };

  window.onMapPinMainNodeMouseDownLeft = function (evt) {
